...
public class MultipleMonitorRoundLayouter : MultipleMonitorLayouter
{
    ...
    [SerializeField]
    private List<Transform> monitorPosition = new List<Transform>();
    void OnDisable()
    {
    ...
    protected override void Layout()
    {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            monitors[i].gameObject.transform.SetParent(null);
        }
        ...
        foreach (var info in monitors) {
            ...
            if (monitorPosition.Count > 0)
            {
                int index = monitors.IndexOf(info);
                info.gameObject.transform.position = monitorPosition[index].position;
            }
        }
    }
