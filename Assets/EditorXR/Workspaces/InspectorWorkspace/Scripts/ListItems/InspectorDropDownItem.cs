...
#pragma warning restore 649
        ...
        void UpdateDropdown()
        {
            ...
            {
                if (m_SerializedProperty.propertyType == SerializedPropertyType.ObjectReference)
                { //ref
                    try
                    {
                        string typeString = m_SerializedProperty.type;

                        Type type = GetTypeByName(typeString);
                        //Type type = Type.GetType("UnityEngine." + typeString + ", UnityEngine");
                        ...
                        {
                            Debug.Log(typeString + " => " + type.ToString());
                        }

                        objects = FindObjectsOfType(type);
                        ...
                        foreach (UnityEngine.Object obj in objects)
                        {
                            try
                            {
                                ...
                                if (obj.Equals(m_SerializedProperty.objectReferenceValue))
                                {
                    ...
                    } catch (System.Exception e)
                    {
                        ...
                        m_DropDown.multiSelect = false;
                        m_DropDown.options = new string[1];
                        m_DropDown.value = 0;
                    }
                }
                else
            ...
            else
            {
                ...
                { //ref
                    try
                    {
                        ...
                        Debug.Log(values[0]);
                        m_SerializedProperty.objectReferenceValue = objects[values[0]-1];
        ...
        private Type GetTypeByName(string typeString)
        {
            //Parse typeString
