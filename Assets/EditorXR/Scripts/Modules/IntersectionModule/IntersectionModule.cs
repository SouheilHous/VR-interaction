...
namespace UnityEditor.Experimental.EditorVR.Modules
{
    sealed class IntersectionModule : MonoBehaviour, ISystemModule, IUsesGameObjectLocking, IGetVRPlayerObjects
    {
                ...
                if (testerTransform.hasChanged)
                {
                    ...
                    if (m_SpatialHash.GetIntersections(m_Intersections, testerCollider.bounds))
                    {
                        ...
                        for (int j = 0; j < m_SortedIntersections.Count; j++)
                        {
                            ...
                            {
                                //Martin's code begins here
                                //if (!renderer.gameObject.name.Contains("Player"))
                                {
                                    //GameObject.FindGameObjectWithTag("ScalingToolManager").SendMessage("NotifyIntersectionTransform", intersection.transform);
                                }
        ...
        internal GameObject GetFirstGameObject(Transform rayOrigin, out float distance)
        {
            try
            {
                // ORIGINAL CODE ADAPTED BY MARTIN TO SUPPORT THE SCALING TOOL MANAGER

                RayIntersection intersection;
                if (m_RaycastGameObjects.TryGetValue(rayOrigin, out intersection))
                {
                    distance = intersection.distance;
                    if (intersection.go != null && intersection.go.transform != null)
                    {
                        GameObject.FindGameObjectWithTag("ScalingToolManager").SendMessage("NotifyIntersectionTransform", intersection.go.transform);
                        GameObject.FindGameObjectWithTag("ScalingToolManager").SendMessage("NotifyIntersectionTransformDistance", intersection.distance);
                    }
                    else
                    {
                        GameObject.FindGameObjectWithTag("ScalingToolManager").SendMessage("NotifyIntersectionTransformNone");
            ...
            try
            {
                // ADDITIONAL CODE BY MARTIN TO DEFAULT TO A PHYSICS RAYCAST IF THE ORIGINAL CODE FAILS

                RaycastHit hitInfo;
                if (Physics.Raycast(rayOrigin.position, rayOrigin.forward, out hitInfo, 1000f))
                {
