...
namespace UnityEditor.Experimental.EditorVR.Utilities
{
    ...
    static class CameraUtils
    {
        public static Camera GetMainCamera()
        {
            ...
            //Find SteamVR Camera
            if (!camera && Application.isPlaying)
            {
                foreach (Camera c in UnityObject.FindObjectsOfType<Camera>())
                {
                    if (c.transform.parent != null && (c.transform.parent.name == "[CameraRig]" || c.transform.parent.name == "VRCameraRig"))
                    {
                        camera = c;
                    }
                }
