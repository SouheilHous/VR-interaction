...
namespace UnityEditor.Experimental.EditorVR.Core
{
    ...
    {
        class DirectSelection : Nested, IInterfaceConnector, IUsesSpatialHash
        {
            ...
            private bool duplicateCurrentlySelectedObjects;
            ...
            internal void UpdateDirectSelection()
            {
                ...
                if (duplicateCurrentlySelectedObjects)
                {
                    for (var i = 0; i < m_DirectSelections.Values.Count; i++)
                    {
                        GameObject g = GameObject.Instantiate(m_DirectSelections.Values.ToList()[i].gameObject);

                        this.AddToSpatialHash(g);


                        if (g.GetComponent<LODGroup>() != null)
                        {
                            foreach (Transform tc in g.GetComponentsInChildren<Transform>())
                            {
                                if (tc != g.transform)
                                {
                    ...
                    duplicateCurrentlySelectedObjects = false;
                }
            }
            ...
            {
                duplicateCurrentlySelectedObjects = true;
            }
