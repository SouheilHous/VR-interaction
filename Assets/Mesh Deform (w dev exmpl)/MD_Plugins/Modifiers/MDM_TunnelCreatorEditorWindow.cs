...
namespace MD_Plugin
{
    public class MDM_TunnelCreatorEditorWindow : EditorWindow
    {
        public static void Init(MDM_TunnelCreator tcc)
        {
            MDM_TunnelCreatorEditorWindow newWin = (MDM_TunnelCreatorEditorWindow)EditorWindow.GetWindow(typeof(MDM_TunnelCreatorEditorWindow));
            newWin.minSize = new Vector2(400, 500);
            newWin.maxSize = new Vector2(500, 550);
            newWin.name = "Tunnect Creator Editor";
            tc = tcc;
            newWin.Show();
        }

        public static MDM_TunnelCreator tc;
        ...
        private void OnSceneGUI()
        {
            ...
            Transform referen = tc.tNodes[iFrom];
            for (int i = iFrom + 1; i <= iTo; i++)
            {
                currentTheta += currentTheta + value0 / c;
                Handles.DrawSphere(0, tc.tNodes[i].transform.position + referen.right * currentTheta, Quaternion.LookRotation(tc.tNodes[i].position - tc.tNodes[i - 1].position), 0.5f);
            }
        }
