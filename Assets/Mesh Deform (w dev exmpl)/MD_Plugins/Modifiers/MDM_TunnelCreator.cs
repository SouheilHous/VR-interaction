...
namespace MD_Plugin
{
    [AddComponentMenu(MD_Debug.ORGANISATION + "/MD Plugin/Modifiers/Tunnel Creator/Tunnel Creator Source")]
    [ExecuteInEditMode]
    public class MDM_TunnelCreator : MonoBehaviour
    {
        ...
        public bool tUpdateEveryFrame = false;
        public bool tEnableSmartRotation = true;

        public bool tUseCustomUVData = false;
        public MDM_TunnelNodeUVData._UVMode tUVMode = MDM_TunnelNodeUVData._UVMode.uvZX;
...
#if UNITY_EDITOR
        [UnityEditor.MenuItem("GameObject/3D Object/MD_Plugin/Advanced/Tunnel Creator")]
        private static void GenerateTunnelObj()
        {
            GameObject newTunnel = new GameObject("TunnelCreator");
            newTunnel.AddComponent<MDM_TunnelCreator>();
        ...
        {
            Transform newOrigin = OctahedronMesh_Generator.Generate(tNodeSize).transform;
        ...
        private void tFunctRefreshMesh(int VertexQueue, int OriginQueue)
        {
            ...
            {
                Matrix4x4 m = Matrix4x4.TRS(OriginPosition.position, OriginPosition.rotation, OriginPosition.localScale);
                Vector3 pos;
        ...
        public void PUBLICtFunct_UpdateUVs(MDM_TunnelNodeUVData._UVMode uvMode)
        {
            for (int v = 0; v < tVertices.Count; v++)
            {
                ...
                switch (uvMode)
                {
                    case MDM_TunnelNodeUVData._UVMode.uvXY:
                        uvModeX = tVertices[v].x;
                        uvModeY = tVertices[v].y;
                        break;

                    case MDM_TunnelNodeUVData._UVMode.uvXZ:
                        uvModeX = tVertices[v].x;
                        uvModeY = tVertices[v].z;
                        break;

                    case MDM_TunnelNodeUVData._UVMode.uvYX:
                        uvModeX = tVertices[v].y;
                        uvModeY = tVertices[v].x;
                        break;
                    case MDM_TunnelNodeUVData._UVMode.uvYZ:
                        uvModeX = tVertices[v].y;
                        uvModeY = tVertices[v].z;
                        break;

                    case MDM_TunnelNodeUVData._UVMode.uvZX:
                        uvModeX = tVertices[v].z;
                        uvModeY = tVertices[v].x;
                        break;
                    case MDM_TunnelNodeUVData._UVMode.uvZY:
                        uvModeX = tVertices[v].z;
                        uvModeY = tVertices[v].y;
                        break;
                }
        ...
        private void tFunctUpdateUVsAtIndex(int originQueue)
        {
            ...
            for (int i = originQueue + 1; i < tNodes.Count; i++)
            {
                if (i >= tNodes.Count)
                    break;
                if (tNodes[i].GetComponent<MDM_TunnelNodeUVData>())
                    break;
                counter++;
            }
            if (counter != 0)
            ...
            MDM_TunnelNodeUVData uvdat;

            if (tNodes[originQueue].GetComponent<MDM_TunnelNodeUVData>())
                uvdat = tNodes[originQueue].GetComponent<MDM_TunnelNodeUVData>();
            else return;
...
namespace MD_Plugin
{
    [CustomEditor(typeof(MDM_TunnelCreator))]
    public class EDITOR_TunnelCreatorEditor : Editor
    {
        MDM_TunnelCreator tc;

        private void OnEnable()
        {
            tc = (MDM_TunnelCreator)target;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button(new GUIContent("Show Tunnel Editor", tc.tc_ICON_ShowW)))
                MDM_TunnelCreatorEditorWindow.Init(tc);
