...
public class MD_VertexTool : EditorWindow
{
    [MenuItem("Window/MD_Plugin/Vertex Tool")]
    ...
    Vector3 MultiplyAngle;
    Vector3 MultiplyMoveOffset = new Vector3(1,0,0);
    ...
    void OnGUI()
    {
        ...
        if (Selection.activeGameObject!= null && Selection.activeGameObject.GetComponent<MD_MeshProEditor>() && Selection.activeGameObject.GetComponent<MD_MeshProEditor>().ppEnablePerformanceSaver)
            if (GUILayout.Button("Group Enabled"))
            {
                MD_MeshProEditor p = Selection.activeGameObject.GetComponent<MD_MeshProEditor>();
                if (p.points.Count==0 || GroupObject == null)
                    return;
                foreach (Transform gm in p.points)
                {
...
{
    /// <summary>
    /// Mesh & vertex tools for internal use
    /// </summary>
    public class MD_VertexToolModifier : MonoBehaviour
    {
        ...
        public static void V_TOOL_Element_Attach(GameObject RootObject)
        {
            ...
            AttachRoot.GetComponent<MD_MeshProEditor>().MeshEditor_QuickCombineMesh();
        }
        ...
        public static void V_TOOL_Element_Clone(GameObject ObjectToClone, int Count, Vector3 RotationAngle, Vector3 MoveOffset)
        {
            ...
            SelectedObj.GetComponent<MD_MeshProEditor>().MeshEditor_ClearVerticeEditor();
            SelectedObj.GetComponent<MD_MeshProEditor>().SelectedModification = MD_MeshProEditor.SelectedModification_.None;
            ...
            SelectedObj.GetComponent<MD_MeshProEditor>().MeshEditor_QuickCombineMesh();
        }
        ...
        public static void V_TOOL_Vertex_Relax(MD_MeshProEditor Sender)
        {
            if (Sender == null)
                return;
            if (Sender.points == null)
                return;
            if (Sender.points.Count > 0)
            {
                foreach (Transform points in Sender.points)
                {
