...
namespace MD_Plugin
{
    [AddComponentMenu(MD_Debug.ORGANISATION + "/MD Plugin/Mesh Paint")]
    public class MD_MeshPaint : MonoBehaviour
    {
        //-----------------------DESCRIPTION------------------------------------------
        //----------------------------------------------------------------------------
        //---MD (Mesh Deformation Collection): Mesh Paint = Component for any object
        //---Full mesh paint manager for any object
        //----------------------------------------------------------------------------
        //----------------------------------------------------------------------------
        //----------------------------------------------------------------------------

        //MESH DATA
        private List<Vector3> internal_vertices = new List<Vector3>();
        private List<int> internal_triangles = new List<int>();
        private List<Vector2> internal_uvs = new List<Vector2>();
        ...
        public bool MP_Platform_Oculus = false;
        ...
        private void Update()
        {
            ...
            if (MP_TypeCustom_CustomBrushTransform && MP_TypeCustom_BrushParent)
            {
                ...
                internal_BrushHelper.rotation = Quaternion.identity;
            }
        ...
        private void INTERNAL_UPDATE_DrawOnCustom()
        {
            ...
            Vector3 rotationdirection = internal_BrushHelper.InverseTransformDirection(MP_TypeCustom_BrushParent.position - internal_ppplastposition);
